...
const snapshot = {
  ...
  recentSongs: [],
  dailyCompletedSongs: Array.from({ length: 7 }, (_, index) => ({ dayStart: `2026-08-${String(25 + index).padStart(2, '0')}T00:00:00Z`, count: index === 6 ? 1 : 0 })),
};

describe('public metrics boundary', () => {
  it('prefers the private HTTPS service reference and recognizes valid zero snapshots', async () => {
    const fetch = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ ...snapshot, totalCompletedSongs: 0, serviceCounts: [], dailyCompletedSongs: [] }) });
  ...
  it.each([
    ...
    [{ ok: true, json: async () => ({ ...snapshot, dailyCompletedSongs: [{ dayStart: 'invalid', count: 1 }] }) }],
  ...
  it('derives factual sharing values and changes its version when displayed data changes', () => {
    expect(summarizePublicMetrics(snapshot)).toEqual({ completedSongs: 4, spotifyLinks: 3, youTubeMusicLinks: 2, lastSevenDaysCompletedSongs: 1 });
