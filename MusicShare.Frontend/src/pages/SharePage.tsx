...
export function SharePage() {
    ...
    // Initialize url directly from query param
    const [url, setUrl] = useState(urlFromQuery || "");
    // Capture whether we had a URL param on mount (never changes after init)
    const [wasAutoSubmit] = useState(!!urlFromQuery);
    // Ref to prevent re-triggering auto-submit
    ...
    // Auto-submit when URL param is present on mount
    useEffect(() => {
        if (
            wasAutoSubmit &&
    ...
    }, [wasAutoSubmit, submitMutation, setSearchParams, url]);
