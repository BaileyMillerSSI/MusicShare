...
For standalone MongoDB compatibility, reconciliation must use deterministic per-share fenced claims rather than a global expiring lease or multi-document transaction. Claim both requested ShareRequest records in ascending ShareId order with one operation token and expiry. Every takeover replaces the token, and every subsequent canonical/alias validation, identity claim, alias CAS, and release filters on that exact token. A stale holder therefore cannot write after expiry/takeover. Historical BSON which omits `reconciliationClaimVersion` is atomically accepted as pre-claim version zero and advanced to one. Failure to claim the second record releases the first by token; a crash leaves only expiring claims. After both claims are held, reload and revalidate the complete pair and bounded incoming-alias rows before any durable reconciliation write. The alias update remains one atomic document CAS. Canonical source-identity backfill may occur only as an idempotent fenced claim before the alias CAS; a crash between those writes must be safe and resumable, never expose a chain/cycle, and never make an unrelated record canonical.
