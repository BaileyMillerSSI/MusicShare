...
Add compare-and-set repository operations that load both candidate shares and their songs/links, mark the alias only if its current reconciliation state still matches the dry-run expectation, and optionally backfill `SourceIdentityKey` onto the canonical request. Retrying the same operation returns success with `changed: false`; attempts to remap an alias, create a cycle, target a missing/non-completed share, or claim an identity owned by a third record fail closed. Query the bounded set of aliases directly targeting either candidate: aliases of the proposed canonical remain direct, but a third-party alias of the proposed alias rejects the operation so `C → A` can never become `C → A → B`.

For standalone MongoDB compatibility, reconciliation must use deterministic per-share fenced claims rather than a global expiring lease or multi-document transaction. Claim both requested ShareRequest records in ascending ShareId order with one operation token and expiry. Every takeover replaces the token, and every subsequent canonical/alias validation, identity claim, alias CAS, and release filters on that exact token. Historical BSON which omits `reconciliationClaimVersion` is atomically accepted as pre-claim version zero and advanced to one. Failure to claim the second record releases the first by token; a crash leaves only expiring claims. After both claims are held, reload and revalidate the complete pair and bounded incoming-alias rows before any durable reconciliation write.

Before the alias CAS, atomically and monotonically pin the selected canonical request as a terminal canonical while its exact claim token is still held. Combine this canonical-role pin with any source-identity backfill in the same fenced update and require that update to match; a lost canonical claim must abort before the alias write. A pinned canonical can accept additional direct aliases but can never later be selected or written as an alias. This durable invariant makes it safe for the final alias CAS to remain a single-document write even if the canonical claim expires afterward. Every durable fenced write must verify its match/update result. A crash after the pin is conservative and resumable: the record remains terminal canonical, and the same or another validated operation may add a direct alias to it without creating a chain or cycle.
...
The service loads both requests, songs, and links; converts each link to the exact identity `(ServiceType, ServiceSongId)`; and requires at least one identical provider identity shared by both songs. Metadata similarity, artwork, duration, title, album, or artist names are supporting display information only and can never authorize a merge. Requests must be completed, distinct, non-conflicting, and resolve to valid `Resolved` songs. Reject Unknown/undefined service values and malformed provider IDs. Query every ShareRequest/SongServiceLink owner of each shared identity and reject the operation if any non-aliased owner exists outside the exact requested pair. Unless the caller explicitly selects one of the validated pair, choose the earliest `ShareRequest.CreatedAt`, then lowercase `ShareId` as the deterministic tie-breaker.

Dry-run returns a bounded plan containing an operation fingerprint/ID, selected canonical share, proposed alias, shared exact identity evidence, current states, and affected counts. Build the fingerprint deterministically from the exact requested ID set; canonical/alias request IDs, Song IDs, statuses, timestamps, alias/claim versions; each resolved Song status/version; and separately sorted provider evidence bound to each Song. It performs no writes, revalidation, or metrics refresh. Apply acquires both fenced claims, reloads the requests/songs/links plus all identity owners, recomputes the complete fingerprint, and requires an exact full-string match before the CAS. It writes the alias metadata, backfills the canonical source identity only when safely fenced/idempotent, revalidates both share paths, and publishes one `RefreshPublicMetrics` message. Log the operation ID, mode, IDs, exact service types, outcome, affected counts, and `changed` state without logging secrets, source URLs, or arbitrary document contents.
...
Add a manual-only GitHub Actions workflow with production-environment protection and inputs for the two share IDs, optional canonical ID, mode (`dry-run` default or `apply`), and dry-run fingerprint. The workflow calls the fixed frontend route with the repository secret, never prints the secret, uploads or summarizes the bounded JSON result including affected counts and bounded provider evidence, and refuses apply without the fingerprint plus an explicit confirmation value. This gives the owner an audited “Run workflow” path and avoids shelling into MongoDB or handling a Mongo connection string. Before shipping, provision a dedicated strong `MAINTENANCE_SECRET` in the GitHub production environment without exposing its value, verify only the secret name, and prove missing/invalid values fail closed in tests.
...
Pass the flag through `PublicMetricsRefreshConsumer` and `IPublicMetricsService` to `PublicMetricsSnapshotRepository`. The repository's ordinary replace path continues to require a non-decreasing total. The reconciliation-authorized path may replace a lower total only when its database-reserved snapshot version is newer than the persisted singleton.

Persist the accepted reconciliation-decrease version as a nullable/omitted-until-set floor on the singleton metrics snapshot. An ordinary candidate must have a reserved version strictly above that floor in addition to satisfying the existing non-decreasing-total rule. Snapshot writes must preserve the floor across later ordinary replacements; an authorized decrease advances it atomically with the snapshot data. This closes both race directions: an older authorized candidate cannot overwrite a newer completion snapshot, and an ordinary candidate reserved before reconciliation cannot resume afterward and restore the stale higher duplicate count. An ordinary candidate reserved after the floor may still publish a newly completed higher total.
...
- `MusicShare.Persistence/Repositories/ShareRequestRepository.cs`: implement named-index duplicate-key recovery, deterministic canonical lookups, alias compare-and-set writes, and alias exclusion in total/recent/daily pipelines.
...
- `MusicShare.Persistence/Entities/PublicMetricsSnapshot.cs`, `MusicShare.Persistence/Repositories/IPublicMetricsSnapshotRepository.cs`, and `MusicShare.Persistence/Repositories/PublicMetricsSnapshotRepository.cs`: add an omitted-until-set reconciliation version floor plus an authoritative newer-version replace path that advances it atomically; preserve the floor on ordinary writes and keep the existing non-regression filter as the default above that floor.
...
- Reproduce the production failure locally by persisting a higher metrics total, then prove an ordinary lower candidate is rejected while a reconciliation-authorized candidate with a newer reserved version is accepted. Test both race directions: an older authorized decrease cannot overwrite a newer completion refresh, and a paused ordinary pre-reconciliation candidate cannot restore the stale higher duplicate count after the authorized decrease. Prove a post-reconciliation ordinary completion above the persisted floor can still increase totals.
...
- Allowing every refresh to decrease totals would weaken the stale-snapshot guard. The exception must be carried explicitly from reconciliation and still require a strictly newer database-reserved version; do not replace the default filter with an unconditional version-only policy. The accepted decrease version must remain as a floor on subsequent ordinary snapshots, or a paused pre-reconciliation higher-total candidate can resurrect the duplicate metrics.
...
- [ ] A successful reconciliation apply can persist and revalidate a lower metrics total through an explicit policy and durable version floor; ordinary lower totals, older authorized candidates, and pre-reconciliation ordinary candidates remain rejected, while post-floor completions can increase totals.
